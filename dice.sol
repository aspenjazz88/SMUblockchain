...
contract Dice {
    ...
    event luckyTimes(uint256 diceId);
    ...
    function stopRoll(uint256 diceId) external ownerOnly(diceId) validDiceId(diceId) {
            ...
            uint8 newNumber = (uint8)((block.timestamp + diceId) % dices[diceId].numberOfSides) + 1;
    ...
    function destroy(uint256 diceId) public ownerOnly(diceId) validDiceId(diceId) {
